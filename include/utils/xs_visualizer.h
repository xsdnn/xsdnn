...
#ifndef XSDNN_XS_VISUALIZER_H
#define XSDNN_XS_VISUALIZER_H
...
class xs_visualizer {
public:
    explicit xs_visualizer(layer *root_node,
...
private:
    ...
    void get_layer_names(node2name_t &node2name) {
        ...
        auto namer = [&](const layer &l) {
            ...
            node2name[&l] =
                    "\"" + ltype + std::to_string(layer_counts[l.layer_type()]++) + "\"";
        };

        dfs_graph_traverse(root_, namer, [&](const edge &) {});
    }
